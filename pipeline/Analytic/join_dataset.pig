...
zillow = LOAD 'local2.txt' USING PigStorage('\t') AS (zpid:chararray, address:chararray, street_address, city, state, zipcode, beds, baths, size:int, sold:int, sale:int, rent, lot, transaction, built, parking);
...
STORE sorted_house INTO 'house_info'
...
STORE all_about_house INTO 'all_about_house';
